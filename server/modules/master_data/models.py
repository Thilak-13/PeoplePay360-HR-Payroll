...
class LeaveType(Base):
    ...
    # Relationships
    leave_allocations = relationship("LeaveAllocation", back_populates="leave_type_rel")
    leave_requests = relationship("LeaveRequest", back_populates="leave_type_rel")
...
class Department(Base):
    ...
    manager_id = Column(
        Integer,
        ForeignKey("employees.id", ondelete="SET NULL", use_alter=True, name="fk_departments_manager_id_employees"),
        nullable=True,
    )
...
class LeaveAllocation(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    employee_id = Column(Integer, ForeignKey("employees.id", ondelete="CASCADE"), nullable=False, index=True)
    holiday_type = Column(String(50), ForeignKey("leave_types.code", ondelete="RESTRICT"), nullable=False, index=True)
    ...
    leave_type_rel = relationship("LeaveType", back_populates="leave_allocations")
...
class LeaveRequest(Base):
    ...
    leave_type_rel = relationship("LeaveType", back_populates="leave_requests")
